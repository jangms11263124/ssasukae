...
import com.ssafy.ssasukae.domain.performance.redis.PerformanceSnapShot;
import com.ssafy.ssasukae.domain.performance.redis.PerformanceStore;
...
public class PerformanceService {
  ...
  public void prepare(
          Long userId,
          Long roomId,
          PerformancePrepareRequest request
  ) {
    if (request == null) {
      throw business(
  ...
  public void startPlayback(
    ...
    saveWithRollback(previous, changed);

    afterCommit(() -> eventPublisher.publish(
            roomId,
  ...
  public void changeSettings(
    ...
    afterCommit(() -> eventPublisher.publish(
            ...
            new PerformanceSettingsChangedPayload(
                    changed.performerParticipantId(),
                    PerformanceSettingsPayload.from(
                            changed.settings()
                    )
            )
    ));
  ...
  public void cancel(
    ...
    Room room =
            validatePlayingRoomAndPerformer(
                    userId,
                    roomId
            );
    ...
    saveWithRollback(previous, cancelled);

    room.cancelPerformance();

    afterCommit(() -> {
      deletePerformance(cancelled);
  ...
  private Room validatePlayingRoomAndPerformer(
          Long userId,
          Long roomId
  ) {
    validatePositive(userId, "userId");
    ...
    if (room.getStatus() != RoomStatus.PLAYING) {
      throw business(
              WebSocketErrorCode.INVALID_ROOM_STATE,
              "현재 공연 중인 방이 아닙니다."
      );
    }
  ...
  private RoomParticipant getOnlinePerformer(
          Long roomId,
          Long userId
  ) {
    RoomParticipant participant =
            roomParticipantRepository
                    ...
                    .orElseThrow(() -> business(
                            WebSocketErrorCode.ROOM_ACCESS_DENIED
                    ));
  ...
  private PerformanceSnapShot getValidatedSnapShot(
          Long roomId,
          Long performanceId,
          Long userId
  ) {
  ...
  private void validateRoomPreparing(Room room) {
    if (room.getStatus() != RoomStatus.PREPARING) {
      throw business(
              WebSocketErrorCode.INVALID_ROOM_STATE,
              "공연을 시작할 수 없는 방 상태입니다."
      );
    }

    if (performanceStore
            .findActiveByRoomId(room.getId())
            .isPresent()) {
      throw business(
              WebSocketErrorCode.PERFORMANCE_ALREADY_IN_PROGRESS
      );
  ...
  private void validatePerformanceResources(Song song) {
    if (!StringUtils.hasText(song.getMrObjectKey())
      ...
      throw business(
              WebSocketErrorCode.PERFORMANCE_RESOURCE_NOT_READY
      );
    }
  }
  ...
  private void saveWithRollback(
    ...
    restoreOnRollback(
            () -> performanceStore.save(previous)
    );
  }
  ...
  private void deletePerformance(
    ...
    try {
      ...
      log.error(
              "공연 Redis 세션 삭제에 실패했습니다. performanceId={}",
              snapShot.performanceId(),
              exception
      );
    }
  ...
  private void restoreOnRollback(Runnable action) {
    if (!TransactionSynchronizationManager
            .isSynchronizationActive()) {
      return;
    }
  ...
  private WebSocketBusinessException invalidPerformanceState(
  ...
  private WebSocketBusinessException business(
    ...
    return new WebSocketBusinessException(
            errorCode,
            message
    );
  }
