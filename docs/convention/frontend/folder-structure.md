...
스택: **Next.js (App Router) + TypeScript**. 아키텍처는 **FSD(Feature-Sliced Design)** 를 따릅니다.

## 레이어 구조

```
src/
├── app/                          # Next.js App Router (라우팅 전용) = FSD app 레이어
│   ├── layout.tsx                # 전역 레이아웃
│   ├── globals.css               # Tailwind 진입 CSS
│   ├── providers/                # 전역 Provider (QueryClientProvider 등)
│   └── (main)/
│       ├── page.tsx              # 라우트 엔트리 — pages 레이어를 불러와 렌더링만 함
│       └── board/
│           └── [id]/
│               └── page.tsx
│
├── pages/                        # 라우트 단위 화면 조립 (Next.js pages 디렉터리와 다름)
│   └── board-detail/
│       ├── ui/BoardDetailPage.tsx
│       └── index.ts
│
...
| 레이어 | 역할 | 예시 |
|---|---|---|
| `app` | 앱 전역 설정, 라우팅 (Next.js App Router와 통합) | `layout.tsx`, `providers/` |
| `pages` | 라우트에 대응하는 화면을 위젯/피처로 조립 | `board-detail/`, `home/` |
...
- `app/**/page.tsx`, `layout.tsx`, `loading.tsx`, `error.tsx`는 **라우팅 전용**으로, 로직 없이 `pages` 레이어의 컴포넌트를 import해서 렌더링만 합니다.
- 실제 화면 조립(데이터 패칭 조합, 위젯 배치 등)은 `pages` 레이어에서 수행합니다.
...
- 상위 레이어만 하위 레이어를 참조할 수 있습니다: `app → pages → widgets → features → entities → shared`
...
| 대상 | 규칙 | 예시 |
|---|---|---|
| 슬라이스 폴더명 | kebab-case | `auth-login/`, `board-detail/` |
...
- `pages` 레이어는 조립만 담당하고 비즈니스 로직을 직접 포함하지 않음
